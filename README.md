...
3. 点击 **连接** 验证
4. 点击 **☁️ 上传** 将数据备份到 Gist

### 3. 跨设备同步

- **上传**：在任意设备录入数据后，点击 **☁️ 上传**（首次会自动创建私密 Gist）
- **下载**：在另一台设备，**只需粘贴同一个 Token 并点击"连接"**，系统会自动找到云端备份 Gist，无需手动记录 Gist ID；随后点击 **📥 下载** 即可
- **合并而非覆盖**：下载时会把云端与本地数据**合并**，本地已抄但云端没有的月份不会被抹掉
